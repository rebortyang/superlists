...
class ListsAndItemModelTest(TestCase):
    ...
    def test_saving_and_retrieving_items(self):
        ...
        second_svae_item = saved_items[1]
        self.assertEqual(first_save_item.text, 'The first (ever) list item')
        self.assertEqual(first_save_item.list, list_)
        self.assertEqual(second_svae_item.text, 'item the second')
        self.assertEqual(second_svae_item.list, list_)


class ListViewTest(TestCase):

    def test_displays_only_items_for_that_list(self):
        current_list_ = List.objects.create()
        Item.objects.create(text='itemey 1', list=current_list_)
        Item.objects.create(text='itemey 2', list=current_list_)
        other_list_ = List.objects.create()
        Item.objects.create(text='other itemey 1', list=other_list_)
        Item.objects.create(text='other itemey 2', list=other_list_)

        response = self.client.get('/lists/%d/' % (current_list_.id,))
    ...
    def test_passes_corrent_list_to_template(self):
        ...
        self.assertEqual(response.context['list'], current_list_)
...
class NewItemTest(TestCase):
    def test_can_save_a_POST_request_to_an_existing_list(self):
        ...
        self.client.post(
            '/lists/%d/add_item' % (current_list.id,),
